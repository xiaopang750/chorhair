...
public class MailUtil {
    ...
    public boolean addremoteFile(String fileurl,String filename){  
    	  System.out.println("增加邮件附件："+fileurl);   
          try{   
              BodyPart bp = new MimeBodyPart();   
     ...
     * @return
     */
    public static boolean sendmoreremotefile(String subject,String to,String content,String files) {
    	MailUtil theMail = new MailUtil(smtp);  
        theMail.setNeedAuth(true); //需要验证  
          
        if(!theMail.setSubject(subject)) return false;  
        if(!theMail.setBody(content)) return false;  
        if(!theMail.addmoreremoteFile(files)) return false;   
        if(!theMail.setTo(to)) return false;  
        if(!theMail.setFrom(from)) return false;  
        theMail.setNamePass(username,password);  
     ...
     * @return
     */
    public static boolean sendsingleremotefile(String subject,String to,String content,String fileurl,String filename) {
    	MailUtil theMail = new MailUtil(smtp);  
        theMail.setNeedAuth(true); //需要验证  
          
        if(!theMail.setSubject(subject)) return false;  
        if(!theMail.setBody(content)) return false;  
        if(!theMail.addremoteFile(fileurl, filename)) return false;   
        if(!theMail.setTo(to)) return false;  
        if(!theMail.setFrom(from)) return false;  
        theMail.setNamePass(username,password);  
