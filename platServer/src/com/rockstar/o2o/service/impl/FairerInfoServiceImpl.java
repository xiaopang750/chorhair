...
public class FairerInfoServiceImpl extends BaseServiceImpl implements FairerInfoService{
	...
	@Override
	public List<FairerInfo> QueryByShop(String province,String city, String county,String pkShop,String querylike,Integer curpage,Integer pagesize) {
		List<FairerInfo> fairerlists = new ArrayList<FairerInfo>();
		Map<String, Object> map = new HashMap<String, Object>();
		StringBuffer buffer=new StringBuffer();
		buffer.append(" from FairerInfo f where IFNULL(f.dr,0)=0 " );
		buffer.append(" and EXISTS (select 1 from ShopInfo  where  IFNULL(dr,0)=0 and pkShop = f.pkShop   " );
		...
		if(pkShop!=null&&!pkShop.equals("")){
			buffer.append(" and f.pkShop = :longpkShop   " );
			Long longpkShop=Long.parseLong(pkShop);
			map.put("longpkShop", longpkShop);
		}
		if(querylike!=null&&!querylike.trim().equals("")){
			buffer.append(" and (f.fairername like '%"+querylike+"%'  or f.nickname like '%"+querylike+"%' or f.cellphone like '%"+querylike+"%') ");
		}
