...
public class BdCustomerAwardServiceImpl extends BaseServiceImpl implements BdCustomerAwardService{
	...
	public List<Object> queryselect(String pkCustomeraward) {
		...
		sqlbuffer.append(" select substr(pk_combo,1,LENGTH(pk_combo)) as pkCombo,comboname,combomoney,(case when selectcombo is not null then 'Y' else 'N' end) as selectcombo,substr(pk_customeraward_limit,1,LENGTH(pk_customeraward_limit)) as pkCustomerawardLimit ");
		sqlbuffer.append(" ,substr(pk_customeraward,1,LENGTH(pk_customeraward)) as pkCustomeraward  from platform_combo a left join  (select c.pk_combo as selectcombo,c.pk_customeraward_limit,b.pk_customeraward  from bd_customer_award b inner join bd_customer_award_limit c ");
