...
public class PlatWxQrcodeScancountServiceImpl extends BaseServiceImpl implements
	...
	public List<WxQrcodeScancount> queryPlatScancountList(Integer curpage,
		...
		if(curpage!=null){
			scancountlists=(List<WxQrcodeScancount>) baseDao.pageQuery("from WxQrcodeScancount a where IFNULL(a.dr,0)=0 order by a.ts desc",curpage,pagesize);
		}else{
			scancountlists=(List<WxQrcodeScancount>) baseDao.pageQuery("from WxQrcodeScancount a where IFNULL(a.dr,0)=0 order by a.ts desc");	
		}
