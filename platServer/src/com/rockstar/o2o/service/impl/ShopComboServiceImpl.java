...
public class ShopComboServiceImpl extends BaseServiceImpl implements ShopComboService{
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<ShopCombo> QueryByShopAndtype(String pk_shop, String fairtype,String combotype) {
		Long longcorp=Long.parseLong(pk_shop);
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<Object> queryByPkCombo(long pkCombo) {
		HashMap<String, Object> map=new HashMap<String, Object>();
		StringBuffer buffer=new StringBuffer();
		buffer.append("select c.pk_shop as pkShop,(SELECT shopname from shop_info where IFNULL(dr,0)=0 and pk_shop=c.pk_shop ) as shopname from shop_combo c where IFNULL(c.dr,0)=0 and c.pk_combo =:pkCombo ");
		map.put("pkCombo", pkCombo);
		List<Object> combolists=(List<Object>) baseDao.querySqlListByConMap(buffer.toString(),map);
	...
	@SuppressWarnings("unchecked")
	@Override
	public ShopCombo queryByShopAndCombo(long pkShop, long pkCombo) {
		List<ShopCombo> combolists=(List<ShopCombo>) baseDao.pageQuery("from ShopCombo where IFNULL(dr,0)=0 and pkShop = ? and pkCombo = ? ",pkShop,pkCombo);
		if(combolists.size()>0){
			return combolists.get(0);
		}
	    return null;
	}
