...
public class ShopProductDeliveryServiceImpl extends BaseServiceImpl implements ShopProductDeliveryService{
	...
	public List<ShopProductDelivery> QueryByShop(String pk_shop,
		...
		if(curpage!=null){
			Deliverylists=(List<ShopProductDelivery>) baseDao.pageQuery("from ShopProductDelivery where IFNULL(dr,0)=0 and pkShop = ? ",curpage,pagesize,longpkshop);
		}else{
			Deliverylists=(List<ShopProductDelivery>) baseDao.pageQuery("from ShopProductDelivery where IFNULL(dr,0)=0 and pkShop = ? ",longpkshop);	
		}
