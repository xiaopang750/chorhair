...
public class CustomerAwardController extends BaseController {
		...
		try {
			...
			if(user!=null&&user.getUsertype().equals("2")){
				...
				while(iter.hasNext()){
					...
					String awardfrom=getobj.get("awardfrom")==null?OrderFrom.FROM_PLAT[0]:getobj.getString("awardfrom");
		...
		try {
			...
			if(user!=null&&user.getUsertype().equals("2")){
				...
				if(oldgroup!=null){
					 ...
					 if(!awards.equals("")){
						 ...
						 while(iter.hasNext()){
							...
							if(!pkCustomeraward.equals("")){
								...
								award.setAwardstatus("001");
								award.setFaittypename(faittypename);
								award.setPkFrom((pkFrom!=null&&!pkFrom.equals(""))?Long.parseLong(pkFrom):null);
								award.setTs(DateUtil.getCurrDate(DateUtil.FORMAT_ONE));
								award.setDr((short)0);
								addlist.add(award);
							}
				 	...
				 	outputstr(addobj.size()>0?JSONArray.fromObject(addobj).toString():"", "维护抵用券成功", true, null);
				}else{
	...
	public void savelimit(HttpServletRequest request,HttpServletResponse response,Model model) {
		try {
			...
			while(iter.hasNext()){
				...
				limit.setCombofrom(OrderFrom.FROM_PLAT[0]);
	...
	public void editlimit(HttpServletRequest request,HttpServletResponse response,Model model) {
		try {
			...
			basicservice.batchoperate(addlimitlists, updatelimitlists);
			outputstr("", "修改奖励限制成功", true, null);
		} catch (Exception e) {
			dealexception(e);
			outputexceptionstr();
		}
