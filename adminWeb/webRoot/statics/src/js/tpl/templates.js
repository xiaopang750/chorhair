...
!function() {
    ...
    }), template.helper("userType", function() {
        return R.userType;
    }), template.helper("timeLeft", function(sEnd, sStart) {
        var sStartTime = new Date(sStart.replace(/\-/gi, "/")).getTime(), sEndTime = new Date(sEnd.replace(/\-/gi, "/")).getTime(), cha = sEndTime - sStartTime, cha1 = parseInt(cha / 1e3 / 60 / 60 / 24, 10);
        return 0 > cha1 && (cha1 = 0), cha1;
