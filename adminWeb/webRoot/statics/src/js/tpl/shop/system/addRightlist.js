/*TMODJS:{"version":14,"md5":"33c28ece7aca1816c42e4396211170a8"}*/
define(function(require) {
    return require("../../templates")("shop/system/addRightlist", function($data) {
        "use strict";
        ...
        return $out += " ", $each(list, function($value, $index) {
            ...
            $out += 1 == $value.israte ? 're="^\\d+$" msg="提成值只能为数字" tip="提成值不能为空"' : 2 == $value.israte ? 're="^(([1-9]\\d?)|100)$" msg="提成比例为100以内的数字" tip="提成比例不能为空"' : 'ignorecheck="yes"', 
            $out += "> ", $out += $escape($value.awardmoney), $out += " </td> <td> ", "look" != type && ($out += ' <a href="javascript:;" aid="', 
            $out += $escape($value.pkFaireraward), $out += '" edit>编辑</a> '), $out += ' <span class="ba-none" sub-name="pkFaireraward" ignorecheck="yes">', 
            $out += $escape($value.pkFaireraward), $out += "</span> </td> </tr> ";
        }), new String($out);
    });
});