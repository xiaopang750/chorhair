/*TMODJS:{"version":221,"md5":"db4a2f6cc4c9b874925b062de70a868e"}*/
define(function(require) {
    return require("../../templates")("shop/user/list", function($data) {
        "use strict";
        var $utils = this, $escape = ($utils.$helpers, $utils.$escape), pkOrder = $data.pkOrder, pkCombo = $data.pkCombo, iscombo = $data.iscombo, customername = $data.customername, cellphone = $data.cellphone, comboname = $data.comboname, servicemoney = $data.servicemoney, ordermoney = $data.ordermoney, fairername = $data.fairername, $each = $utils.$each, addition = $data.addition, discount = ($data.value2, 
        $data.index2, $data.$value3, $data.$index3, $data.discount), $out = "";
        return $out += ' <li sum-list class="ba-rel" pid="', $out += $escape(pkOrder), $out += '" pkcombo="', 
        $out += $escape(pkCombo), $out += '"> <span class="packType"> ', $out += iscombo ? ' <span class="ba-red"> 套餐订单 </span> ' : ' <span class="ba-qing"> 服务订单 </span> ', 
        ...
        $out += $escape(discount), $out += '"> <span class="ba-fl ba-mt-5">元</span> </td> </tr> <tr> <td> 结算方式： </td> <td> <select class="form-control paymode" paymode> <option value="1">现金</option> <option value="2">刷卡</option> <option value="3">微信</option> <option value="4">支付宝</option> </select> <a class="btn btn-warning dosage" href="javascript:;" dosage>用量</a> </td> </tr> </table> <div class="ba-clearfix"> <span class="ba-fl ba-mt-5"> 总计: <span sum-all org="', 
