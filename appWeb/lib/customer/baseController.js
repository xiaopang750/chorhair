...
module.exports = function(app) {
	...
	//公共数据
	var commonData =  {};
	var viewConfig = require('../../config/customer/view');
	var allway = app.get('allway');
	var http = require('http');
	var qs = require('querystring');
	var siteNav = require('../../config/customer/nav');
	var inter = require('../../config/customer/api/index');
	...
	var baseView = app.get('Class').create({
		...
		checkIsLogin: function(req, res) {
			...
			if(req.cookies.pkUser && !req.session.uid ) {
				...
				var nowShortUrl = req._parsedUrl.pathname.replace('/views/customer/', '');
		...
		save: function(req, res, url, param, suc, fail) {
			...
			param.usergroup = app.get('allway')['001'].id;
