...
module.exports = function(app) {
	...
	//导航配置
	var siteNav = require('../config/nav');

	//渠道配置
	// var allway = app.get('allway');

	//接口配置
	var inter  = require('../config/api/index');
	...
	commonData.javaDomain = app.get('javaDomain');
	commonData.javaPort = app.get('javaPort');
	commonData.downloadDomain = app.get('downloadDomain');
	...
	commonData.nowVersion = app.get('nowVersion');
	...
	var baseView = app.get('Class').create({
		...
		loadView: function(opts) {
			...
			var nowPageName = route.substring(7);
		...
		save: function(req, res, url, param, suc, fail, hostname, port) {
		...
		request: function(req, res, url, param, suc, fail, hostname, port) {
            ...
            if(req.cookies.pkUser){
            	param.pkUser = req.cookies.pkUser;
            }else{
            	param.pkUser = pkUser;
            }
