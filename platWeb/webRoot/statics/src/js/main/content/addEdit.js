...
define(function(require, exports, module) {
	...
	var ueImage = require('../../widget/ueditor/image');
	...
	var AddEdit = R.Class.create(R.util, {
		...
		initialize: function() {
			...
			var oUeImage = new ueImage({
				oUploadBtn: $('[upload-btn-haha]'),
				confirmBtn: $('[upload-btn-confirm]'),
				ue: this.ue
			});
		...
		shopFair: function() {
			...
			var _this = this;
			var contenttype = bodyParse().contenttype;
			// var pkShop = bodyParse().pkShop;

			if(contenttype == 'shop'){
				this.reqUrl = R.interfaces.content.search;
				this.reqParam = {
					pagesize: '10',
					curpage: '1',
					contenttype: 'fairer'
				...
				this.req(function(data) {
					...
					_this.fairList = JSON.stringify(data);
		...
		/*getSignature: function(contenturl, cb) {

			var _this = this;

			this.reqUrl = R.interfaces.global.getSignature;

			this.reqParam = {
				noncestr: 'Wm3WZYTPz0wzccnW',
				timestamp: 1427939312,
				url: contenturl
			}; 

			this.req(function(data) {
				_this.signature = data.data.sign;
				cb && cb();
			},function(data) {
				console.log(data);
			});
		...
		subForm: function(data) {
			...
			//模板类型
			this.reqParam.contentType = bodyParse().contenttype;

			//理发师所属店铺
			this.reqParam.pkShop = bodyParse().pkShop;
			//地址
			this.reqParam.address = bodyParse().address;
			//店名
			this.reqParam.shopname = bodyParse().shopname;
			//经纬度
			this.reqParam.location = bodyParse().location;
			//营业时间
			this.reqParam.time = bodyParse().time;
			//电话
			this.reqParam.phone = bodyParse().phone;
			//理发师列表
			this.reqParam.fairList = this.fairList;
