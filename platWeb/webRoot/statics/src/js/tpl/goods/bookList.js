/*TMODJS:{"version":1,"md5":"4880b07d60fc5b35b74269060e343eda"}*/
define(function(require) {
    return require("../templates")("goods/bookList", function($data) {
        "use strict";
        ...
        return $out += " ", list ? ($out += " ", $each(list, function($value) {
            ...
            $out += '&sc=edit" operation="edit">编辑</a> <a class="btn btn-normal" href="javascript:;" bid=', 
            ...
            $out += $string(getRoute([ "goods/bookAddEdit" ])), $out += "?bid=", $out += $escape($value.pkProductBook), 
            $out += "&status=", $out += $escape($value.vbillstatus), $out += "&note=", $out += $escape($value.note), 
            $out += '&sc=edit" operation="edit">编辑</a> '), $out += ' <a class="btn btn-normal" href="', 
            $out += $string(getRoute([ "goods/bookAddEdit" ])), $out += "?bid=", $out += $escape($value.pkProductBook), 
            $out += "&status=", $out += $escape($value.vbillstatus), $out += "&note=", $out += $escape($value.note), 
