...
public class CustomerOrderServiceImpl extends BaseServiceImpl implements CustomerOrderService{
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<CustomerOrder> FindByShopAndDateAndPay(String pkShop, String begintime,String endtime,
			String paystatus,Integer curpage,Integer pagesize) {
		// TODO Auto-generated method stub
		Long longpkShop=Long.parseLong(pkShop);
		List<CustomerOrder> orderlist=new ArrayList<CustomerOrder>();
		Map<String, Object> map = new HashMap<String, Object>();
		StringBuffer buffer=new StringBuffer();
		buffer.append(" from CustomerOrder where IFNULL(dr,0)=0 and pkShop = :longpkShop  and orderstatus = :orderStatus " );
		map.put("longpkShop", longpkShop);
		map.put("orderStatus", paystatus);
		
		 if(begintime!=null&&!begintime.equals("")){
			 buffer.append(" and ordetime >= :orderbegintime ");
			 
			 map.put("orderbegintime", begintime);
		 }
		 
		 if(endtime!=null&&!endtime.equals("")){
			 buffer.append(" and  ordetime <= :orderendtime ");			 
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<CustomerOrder> querybycon(String pkShop, Integer curpage,
			Integer pagesize, String begintime, String endtime, String status) {
		// TODO Auto-generated method stub
		Long longpkShop=Long.parseLong(pkShop);
		StringBuffer buffer=new StringBuffer();
		Map<String, Object> map = new HashMap<String, Object>();
		buffer.append("from CustomerOrder a where IFNULL(a.dr,0)=0 and pkShop = :Shop and orderstatus = :Orderstatus  ");
	...
	@SuppressWarnings("unchecked")
	@Override
	public int FindShopDayOrder(String pkShop, String getdate) {
		// TODO Auto-generated method stub
		Long longcorp=Long.parseLong(pkShop);
		String sql="select count(pk_order)  from  customer_order where  IFNULL(dr,0)=0  and pk_shop = ? and substr(ordetime,1,10) = ? and orderstatus in ('001','002') ";
		List<Object> obj= (List<Object>) baseDao.querybysql(sql, longcorp ,getdate);
		if(obj.size()>0){
			int count=Integer.parseInt(obj.get(0).toString());
			return count;
