...
public class DateUtil {
	...
	/** 格式：年－月－日 **/
	public static final String LONG_DATE_NOFORMAT = "yyyyMMdd";
	 ...
	 * @return
	 */
	public static long getSecond(String date1, String date2) {
		if (date1 == null || date1.equals(""))
			return 0;
		if (date2 == null || date2.equals(""))
			return 0;
		...
		long second = (mydate.getTime() - date.getTime()) / 1000;
		return second;
	}
	...
	public String getPreviousWeek() {
		...
		Calendar lastDate = Calendar.getInstance();
		lastDate.add(Calendar.WEEK_OF_YEAR, -1);// 减一周
		str = sdf.format(lastDate.getTime());
		return str;
	}
	// 获得本周是全年第几周
	...
	// 获得上周星期日的日期
	public String getPreviousWeekSunday2() {
		weeks = 0;
		weeks--;
		int mondayPlus = this.getMondayPlus();
		GregorianCalendar currentDate = new GregorianCalendar();
	...
	// 获得上周星期一的日期
	public String getPreviousWeekday2() {
		weeks--;
		int mondayPlus = this.getMondayPlus();
		GregorianCalendar currentDate = new GregorianCalendar();
		currentDate.add(GregorianCalendar.DATE, mondayPlus + 7 * weeks);
	...
	// 获得本年第一天的日期(2015-01-01 00:00:00)
	public String getCurrentYearFirst2() {
		int yearPlus = this.getYearPlus();
		GregorianCalendar currentDate = new GregorianCalendar();
		currentDate.add(GregorianCalendar.DATE, yearPlus);
		Date yearDay = currentDate.getTime();
	...
	// 获得上年第一天的日期 *
	public String getPreviousYearFirst2() {
		Date date = new Date();
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy");// 可以方便地修改日期格式
		String years = dateFormat.format(date);
		int years_value = Integer.parseInt(years);
	...
	//	 获得参数月的本季度最后一天
	public String getThisSeasonEndDay(int month) {
		String array[][] = { { "01", "02", "03" }, { "04", "05", "06" }, { "07", "08","09" }, { "10", "11", "12"} };
		int season = 1;
		if (month >= 1 && month <= 3) {
			season = 1;
		...
		String years = dateFormat.format(date);
		int years_value = Integer.parseInt(years);
		int end_days = getLastDayOfMonth2(years_value,end_month);
		String seasonDate = years_value + "-" + end_month + "-" + end_days;
		return seasonDate;
	}
	//获得参数月的上季度第一天
	public String getPreviousSeasonFirstDay(int month) {
		String array[][] = { { "01", "02", "03" }, { "04", "05", "06" }, { "07", "08","09" }, { "10", "11", "12"} };
		int season = 1;
		if (month >= 1 && month <= 3) {
			season = 4;
		...
		int years_value=0;
		if(season==4){
			years_value= Integer.parseInt(years)-1;
		}else{
	...
	//	 获得参数月的上季度最后一天
	public String getPreviousSeasonEndDay(int month) {
		String array[][] = { { "01", "02", "03" }, { "04", "05", "06" }, { "07", "08","09" }, { "10", "11", "12"} };
		int season = 1;
		if (month >= 1 && month <= 3) {
			season = 4;
...
//	 获得参数月的上季度是哪一年
	public int getPreviousSeasonYear(int month) {
		int season = 1;
		if (month >= 1 && month <= 3) {
			season = 4;
		}
	 ...
	 * @return 最后一天
	 */
	private int getLastDayOfMonth2(int year, String month) {
	...
	// 获得本季度是几季度
	public int getThisSeason(int month) {
		int season = 1;
		if (month >= 1 && month <= 3) {
			season = 1;
		}
	...
	// 获得上季度是几季度
	public int getPreviousSeason(int month) {
		int season = 1;
		if (month >= 1 && month <= 3) {
			season = 4;
		}
